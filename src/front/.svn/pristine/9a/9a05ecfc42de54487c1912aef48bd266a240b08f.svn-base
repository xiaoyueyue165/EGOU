appcan.define("listview_new", function($, exports, module) {
...
<%}%>\
    ...
    Listview_new.prototype = {
        buildListview_New : function(data,group) {
            var container = $("<ul></ul>")
            var self = this;
        ...
        buildGroupview:function(data){
            ...
            for(var i in data){
                ...
                con.append(ele);
                con.append(self.buildListview_New(data[i].items,i));
            }
            return con;
        },
        add : function(data, dir) {
            var self = this;
            var container = self.buildListview_New(data);
            if (dir || dir == 1)
        ...
        set : function(data) {
            ...
            if(self.option.hasGroup){
                ...
                container = self.buildListview_New(data);
    ...
    module.exports = function(option) {
        return new Listview_new(option);
    };
});
